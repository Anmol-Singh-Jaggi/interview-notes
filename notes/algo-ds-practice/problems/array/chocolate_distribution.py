...
Basically sort the array.
Now make a window of size k.
Compute max - min of every such window.
The window whose max - min if minimum is our answer.
Note that getting max and min are trivial since min is always the first element of the window,
and max is the last element.
'''
